...
const serverInstances: Map<string, ServerInstance> = new Map()
...
async function startServerForFolder(context: vscode.ExtensionContext, folder: vscode.WorkspaceFolder): Promise<void> {
  ...
  try {
    ...
    const client = new LanguageClient('novelsaga', `NovelSaga Language Server (${folder.name})`, run, clientOptions)
    ...
    void vscode.window.showErrorMessage(`Failed to start NovelSaga server for ${folder.name}: ${error as Error}`)
  }
}

async function stopServerForFolder(folder: vscode.WorkspaceFolder): Promise<void> {
  const folderKey = folder.uri.toString()
